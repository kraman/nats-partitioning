...
func Create(config EtcdManagerConfig) (manager.PartitionManager, error) {
...
func (m *partitionManager) GetOwner(partitionKey string) *cluster.Member {
	m.Lock()
	defer m.Unlock()
	return nil
}

func (m *partitionManager) Invoke(partitionKey string, f func(partitionOwner *cluster.Member)) {
	owner := m.GetOwner(partitionKey)
	f(owner)
}
...
func (m *partitionManager) processedReleases(msg *clientv3.Event, acquireCh chan int, releaseCh chan int) {
...
func (m *partitionManager) processedAssigned(assignedInfo []byte, acquireCh chan int, releaseCh chan int) {
...
func (m *partitionManager) assignPartitions(msg *cluster.MemberEvent) {
	...
	if getResp.Count != 0 && string(getResp.Kvs[0].Value) == string(b) {
		m.Logger.Debug("skipping update %s", string(b))
		return
	}
	m.Logger.Debug("update assignment %s", string(b))
...
func (m *partitionManager) updateOwners() {
	...
	m.processedAssigned(getResp.Kvs[0].Value, acquireCh, releaseCh)
	...
	for {
		...
		case partitionID := <-acquireCh:
			...
			if getResp.Count == 0 {
				...
				_, err = m.EtcdConn.Put(
					timeoutCtx,
					fmt.Sprintf("partition/%s/%d", m.Cluster.Name(), partitionID), m.Cluster.ID().String(),
					clientv3.WithLease(m.leaseID),
				)
				if err != nil {
					m.Logger.Fatal(err)
				}
				m.Logger.Infof("partition %d acquired", partitionID)
			}
		...
		case etcdMsg := <-updatesCh:
			for _, msg := range etcdMsg.Events {
				if strings.HasSuffix(string(msg.Kv.Key), "assigned") {
					m.processedAssigned(msg.Kv.Value, acquireCh, releaseCh)
				} else {
					m.processedReleases(msg, acquireCh, releaseCh)
				}
			}
		case msg := <-memberCh:
