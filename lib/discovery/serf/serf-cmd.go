...
var serfCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		s, err := New(ip, ip.String(), gossipPort, gossipPeers)
		if err != nil {
			logrus.Fatal(err)
			return
		}
...
func init() {
	serfCmd.Flags().IP("ip", net.ParseIP("127.0.0.1"), "IP of this server")
	serfCmd.Flags().IPSlice("gossip-peers", nil, "one or more peer IPs")
	serfCmd.Flags().Int("gossip-port", 1770, "gossip port")
	serfCmd.Flags().Int("raft-port", 1771, "raft port")
	serfCmd.Flags().String("raft-dir", "", "raft data dir")
	cmd.RootCmd.AddCommand(serfCmd)
}
